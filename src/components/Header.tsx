...
import { Stethoscope, User, LogIn, LogOut, Menu, X, Shield, LayoutDashboard, ArrowLeft, ArrowRight, Sun, Moon } from "lucide-react";
...
export default function Header({ currentView, setCurrentView, userProfile, onLogout, themeMode = "light", onToggleTheme }: HeaderProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            {/* Functional Back & Front Arrows triggering window.history.back() and window.history.forward() */}
            <div className={`flex items-center space-x-1 pr-2 sm:pr-3 border-r ${isDark ? "border-zinc-800" : "border-zinc-200"}`} id="header-history-arrows">
              <button
                onClick={() => window.history.back()}
                title="Go Back (Previous Page)"
                aria-label="Go Back"
                className={`p-1.5 sm:p-2 rounded-lg transition-all cursor-pointer flex items-center justify-center border shadow-2xs ${
                  isDark 
                    ? "text-zinc-300 hover:text-white hover:bg-zinc-800 active:bg-zinc-700 border-zinc-800 bg-zinc-900/60" 
                    : "text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100 active:bg-zinc-200 border-zinc-200/80 bg-zinc-50/50"
                }`}
                id="btn-history-back"
              >
          ...
          <div className="hidden md:flex items-center space-x-3">
            {/* Theme Toggle Button placed right close to Patient Portal / Username */}
            {onToggleTheme && (
              <button
                onClick={onToggleTheme}
                title={isDark ? "Switch to Brightness (Light Mode)" : "Switch to Darkness (Dark Mode)"}
                aria-label="Toggle Theme Mode"
                className={`p-1 rounded-2xl border transition-all cursor-pointer shadow-2xs flex items-center space-x-1 ${
                  isDark 
              ...
              >
                <div className={`px-2.5 py-1 rounded-xl transition-all flex items-center space-x-1.5 ${
                  ...
                  <span className="text-[10px] font-mono tracking-tight uppercase">Brightness</span>
                </div>
                <div className={`px-2.5 py-1 rounded-xl transition-all flex items-center space-x-1.5 ${
                  ...
                  <span className="text-[10px] font-mono tracking-tight uppercase">Darkness</span>
                </div>
              </button>
            )}
