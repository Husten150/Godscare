  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === "auth/operation-not-allowed") {
        ...
        setErrorMsg("Google Auth popup was restricted by the browser preview frame. Please open the app in a new tab or use Email & Password sign-in below.");
      } else {
    ...
    <div className={`py-8 sm:py-12 px-4 sm:px-6 lg:px-8 font-sans min-h-[85vh] flex flex-col justify-center items-center transition-colors ${
      ...
      <div className={`max-w-6xl w-full rounded-2xl border shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[620px] transition-all ${
        ...
        <div className={`lg:col-span-6 xl:col-span-5 p-6 sm:p-10 flex flex-col justify-center border-t lg:border-t-0 lg:border-l transition-colors ${
          ...
          {/* Alerts */}
          {errorMsg && (
            <div className="flex flex-col space-y-2 bg-rose-500/10 text-rose-400 text-xs p-3.5 rounded-xl border border-rose-500/30 font-sans mb-4">
              <div className="flex items-center space-x-2">
                <AlertCircle className="h-4 w-4 shrink-0 text-rose-500" />
                <span className="leading-relaxed">{errorMsg}</span>
              </div>
              {(errorMsg.includes("preview frame") || errorMsg.includes("popup") || errorMsg.includes("new tab") || errorMsg.includes("restricted")) && (
                <button
                  type="button"
                  onClick={() => window.open(window.location.href, "_blank")}
                  className="mt-1 py-1.5 px-3 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-extrabold rounded-lg text-[11px] font-mono uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all cursor-pointer self-start shadow-sm"
                >
